...
def main(opts):
    ...
    for epoch_idx in range(opts.n_epoches):
        ...
        for i, data in enumerate(tqdm(train_loader, 0)):
            ...
            H_0 = torch.eye(4).repeat(B, 1, 1).to(opts.device)
            H_0[:, :3, :3], H_0[:, :3, 3] = Rs_gt, ts_gt
            H_T = torch.eye(4).repeat(B, 1, 1).to(opts.device)
            ...
            # Compute Chamfer loss for this batch
            train_losses_chamfer = compute_losses_diff(
                opts, X, X_t, [pred_transforms], data['transform_gt'], loss_type="chamfer", reduction='mean'
            )
            rcd["chamfer_losses"].append(train_losses_chamfer['total'].item())
        ...
        print(opts.results_dir)
        print("=== Train. Epoch [%d], Chamfer loss: %1.6f ===" % (epoch_idx, cal(rcd["chamfer_losses"])))
